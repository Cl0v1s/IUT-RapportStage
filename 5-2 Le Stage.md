...
En examinant les différents élements constituant ledit système, On trouve donc un panel d'applications (de type client lourd et client léger) relativement hétéroclyte fonctionnant avec différentes versions du runtime .NET, des versions 1.1 à 3.5.

![.NET](res/net.png)
* L'architecture du Framework.NET 

Bien que la rétrocompatibilité fut majoritairement assurée au fil des versions, il était des plus intéressant, en 2016, de migrer l'ensemble du code pour qu'il fonctionne avec une version plus récente de l'environnement d'exécution de Microsoft, et ce afin de gagner en homogénéité mais aussi en performances ainsi qu'en terme de pérénité du parc applicatif. Concernant ce dernier point, il peut paraître important de préciser que les applications web furent développer pour fonctionner sur NETSCAPE (ayant totalement disparu aujourd'hui) ainsi que sur Internet Explorer 5. Il était donc relativement urgent, pour des raisons de sécurité notamment de faire évoluer le code. 

![ASP](res/asp.png)
* Fonctionnement et organisation d'ASP.NET

Cette mise à jour des sources présentera quelques problèmes, regroupables en trois grandes catégories:
...
* Les architectures utilisées pour organiser les projets étaient également vieillissantes, il était bon de les remettre "au goût du jour" afin de garantir une meilleure maintenabilité future.

Durant cette période de trois mois, il s'agissait donc, concernant le code, de convertir les applications, de corriger les problèmes issus de cette convertion mais également de, si possible, d'améliorer l'existant, ensemble catégorisable en trois grandes tâches.

* Migrer le portail intranet
* Migrer les applications web de l'intranet
* Migrer les clients lourds
* Améliorer le portail et les applications en général 

![Le portail](res/portal.png)
* Une capture d'écran du portail après migration



### Concernant le serveur 

**L**'intranet fut hébergé pendant plus de 10 ans sur un serveur (physique de 2003 à 2008 puis virtuel de 2009 à 2016) fonctionnant sous Windows Server 2003, exécutant le gestionnaire de base de données SQL Server 2008 (Chronologiquement SQL Server 2000, 2005 puis 2008) ainsi qu'IIS 6 (Internet Information Server), la solution de serveur Web de Microsoft. Il s'agissait donc, toujours dans le cadre de cette mise à jour importante, de transférer l'ensemble des applications sur une nouvelle machine Windows Server 2012 R2 prévue au préalable par M. Fernandez, équipée d'IIS 7 ainsi que de SQL Server 2012. Les objectifs qui me furent affectés ici sont:
...
* Installer sur le nouveau serveur l'ensemble des tâches planifiées d'administration (copie de données, synchronisation de bases etc...) 

Ces tâches, nécessaires à la réalisation de la mise à jour du code furent exécutées en priorité, et durant les premiers jours du stage. 

## Des responsabilités certaines

**L**'ensemble des logiciels de l'intranet est indispensable au bon fonctionnement de la SGAC. En effet, ceux-ci sont utiles à tout les corps de métiers, des fonctions supports (Gestion des ressources humaines, Services généraux...) aux services au coeur de l'activité de la société d'assainissement (maintenance réseau, gestion usine de traitement etc..). Aussi, toute interruption du fonctionnement des logiciels, de quelque nature qu'elle soit (erreur de manipulation des machines, applications bugguées) durant les heures de travail des salariés (8H30 - 16H30) n'est pas envisageable. Aussi, il fut nécessaire d'apporter un soin tout particulier à la fluidité de la migration afin de limiter l'impact sur les habitudes des utilisateurs, objectif atteint avec plus ou moins de succès. 
...
L'intranet est dans les faits constitué de deux groupes d'applications distincts, en plus du portail. 

Nous constatons la présence, d'une part, des applications de type client léger qui permettent à la fois aux différents services d'atteindre leurs objectifs mais une une meilleur transmission de l'information au sein de l'entreprise tout entière (ce qui permet des gains de productivité notables, tout en aidant à la prise de décision). 
...
|-----|-------|----------|
| Portail | Client léger | Zone d'accès permmetant d'atteindre l'ensemble des applications de l'intranet |
| Bureau Métier Mobile | Client lourd | Permet la consultation de différentes données sans passer par l'intranet. Présente des fonctionnalités de fonctionnement hors connexion et de synchronisation avec les bases de données de l'intranet (base de données déportée). | 
| Rapports | Client léger | S'interface avec SQL Server Report Server, permet de consulter les rapports depuis l'intranet |
| Gestion Temps | Client léger | Application de pointage permettant aux salariés de déclarer leurs heures |
| Indicateurs | Client léger | Permet de consulter certains indicateurs quant à l'état du réseau |
| Application métiers | Client léger | Outil de travail du traitement des déchets |
| Qualité Sécurité | Client léger | Application de contrôle des procédures qualité et sécurité | 
| AgenceASA | Bibliothèque | Contient des fonctions relatives au traitement des déchets |
| CoActivitéUsine | Client léger | Permet la coordination des interventions des sous-traitants |
| MaximoMetro | Client léger | Présentation et gestion des bons de travail usine |
| Reporting | Rapports | Ensemble de rapports SQL Report Server |
| BdlIppop | Client léger + bibliothèque | Gestion du patrimoine de l'entreprise |
| ExtranetDoea | Client léger | Visionneur d'informations à destination de l'extranet |
| Patrimoine | Bibliothèque | Contient des fonctions relatives à la gestion du patrimoine |
| ServicesWebPatrimonio | Client léger | Présente des webservices relatifs au patrimoine utilisés par d'autres applications |
...
* Comprendre la logique du code/développeur ayant produit ces élements
...
Une fois ces applications testée, il s'agissait de mettre à exécution la migration, en transférant le serveur intranet(ensemble portail, applications et tâches planifiées) sur la nouvelle machine, jusqu'alors serveur de développement.   
...
![Plan migration](res/migration.png)
...
Celle-ci, sobrement nommée "Application Intranet", permet d'accéder depuis les postes utilisateurs à des informations présentes sur le site. Elle rassemble un client IPPOP (base de donnée patrimoine d'entreprise), un client Métrologie (à l'architecture plus que complexe) ainsi qu'une interface présentant un ensemble de rapports (utilisant SQL Report Server précedemment présenté).

![Métrologie](res/metro.png)
...
Les standards du web sont mis en point par le W3C, organisme chargé de veiller au bon développement des technologies de l'internet. Relativement stables depuis l'apparition de HTML5, la façon dont traitaient Javascript les navigateur auparavant étaient plus qu'aléatoire. Le code ayant été prévu pour fonctionner sur une vieille version d'Internet Explorer (5.5), une quantité assez importante du code javascript présent dans les pages est problématique. L'essentiel des corrections de bugs rapide ayant eu lieu vers la fin du stage est lié à des problèmes d'interprétation du langage de script coté client. 

## La documentation

Constatant le temps perdu à comprendre le fonctionnement de l'Intranet, et les problèmes liés à la non documentation de celui-ci, une des tâches qui me fut confié était de réaliser un certain nombre de guide d'installation, concernant certaines applications récalcitrantes, ainsi que de cartographie les interdépendance, tout en assurant une certaine documentation de mes réalisations. 

Ainsi, entre autre, furent produite une cartographie des interdépendances entre applications  et un schéma présentant l'organisation du code du portail, largement modifié par mes soins:

![Organisation du code du portail](res/orga.png)
* Organisation du code du portail

# L'objectif secondaire: La gestion des demandes faites aux services généraux

## Description du besoin

Afin d'assurer le confort des salariés, leur productivité et le bon fonctionnement de l'entreprise, la SGAC dispose de services généraux, chargés d'intervenir, à la demande des salariés sur l'ensemble des problématiques matérielles (hors informatique).  
...
Cependant, suite à des discussions internes à l'équipe du service informatique, en relation avec les services généraux et en vue de la complexité d'intégration du logiciel de ticketing (qui n'est pas compatible avec le système de gestion d'authentification et des droits de la SGAC), T.Fernandez m'a proposé de développer cet outil, m'en jugeant capable. Ayant constaté qu'il n'est pas toujours aisé (ou agréable) de reprendre le code d'un autre développeur, la perspective de développer une application de A à Z et de l'intégrer au système m'enthousiasma au plus haut point. 
...
Etant donné que la production de documents n'est pas prioritaire au vu de la culture du service informatique, l'essentiel de la conception se réalisa en se basant sur de simples notes et sur la collaboration rapprochée des services généraux, futurs utilisateurs de l'application et jouant ici le rôle de "clients". La production en elle-même est en revanche documentée. 
...
Avant de débuter la réalisation de la solution, une courte phase de conception en réunion s'imposa durant laquelle modèle de donnée, apparence et choix technologique furent effectués.
...
ASP.NET permet la création de pages web divisées en deux sous-ensembles. D'une part, nous avons le code de l'interface, en HTML mélé à des élements spécifiques à ASP, et d'autre part le codebehind, responsable du traitement des données et de faire répondre l'interface aux interractions des utilisateurs. 

![Code Behind](res/behind.gif)
* Fonctionnement interprétation/interprétation d'une page ASP.NET
...
![Documentation XML](xml.png)
* Exemple de commentaire permettant la génération d'une documentation technique

#### Le résultat final

![Ajout d'une demande](res/addticket.png)
* Interface d'ajout d'une nouvelle demande

![Interface de traitement](res/operation.png)
![Interface de traitement](res/details.png)
* Interfaces de traitement des tickets

![Interface de statistiques](res/stats.png)
